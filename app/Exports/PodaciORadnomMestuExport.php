...
class PodaciORadnomMestuExport implements FromQuery, WithHeadings, WithMapping
{
    ...
    public function headings(): array
    {
        return [
            ...
            'Број усвојених жалби на решење о пријему у радни однос',
            ...
            'Дан објављивања листе кандидата који су испунили мерила у изборном поступку',
            ...
            'Следеће рангирани кандидат је из',
            'Број бодова следеће рангираног кандидата на ОФК',
            'Број бодова следеће рангираног кандидата на ПФК',
            'Број бодова следеће рангираног кандидата на ПК',
            'Број бодова следеће рангираног кандидата на завршном разговору',
    ...
    public function map($row): array
    {
        return [
            ...
            $row->mestaRada->pluck('grad')->join(', '),
            $row->statusKonkursaRelation?->status_konkursa,
            $row->datum_ishoda_konkursa ? Carbon::parse($row->datum_ishoda_konkursa)->format('d.m.Y') : '',
            $row->razlogNeuspelogKonkursaRelation?->razlog,
            $row->datum_dobijanja_saglasnosti_vlade ? Carbon::parse($row->datum_dobijanja_saglasnosti_vlade)->format('d.m.Y') : '',
            $row->datum_donosenja_resenja_o_pokretanju_postupka ? Carbon::parse($row->datum_donosenja_resenja_o_pokretanju_postupka)->format('d.m.Y') : '',
            $row->datum_dobijanja_obavestenja_od_suka ? Carbon::parse($row->datum_dobijanja_obavestenja_od_suka)->format('d.m.Y') : '',
            $row->datum_odrzavanja_prvog_sastanka ? Carbon::parse($row->datum_odrzavanja_prvog_sastanka)->format('d.m.Y') : '',
            $row->datum_oglasavanja ? Carbon::parse($row->datum_oglasavanja)->format('d.m.Y') : '',
            $row->datum_pregleda_prijava ? Carbon::parse($row->datum_pregleda_prijava)->format('d.m.Y') : '',
            $row->broj_kandidata_za_koje_se_zakazuju_ofk,
            $row->datum_pocetka_provere_ofk ? Carbon::parse($row->datum_pocetka_provere_ofk)->format('d.m.Y') : '',
            $row->datum_ofk_izvestaja ? Carbon::parse($row->datum_ofk_izvestaja)->format('d.m.Y') : '',
            $row->broj_kandidata_za_koje_se_zakazuju_pfk,
            $row->datum_pocetka_provere_pfk ? Carbon::parse($row->datum_pocetka_provere_pfk)->format('d.m.Y') : '',
            $row->datum_pfk_izvestaja ? Carbon::parse($row->datum_pfk_izvestaja)->format('d.m.Y') : '',
            $row->broj_kandidata_za_koje_se_zakazuju_pk,
            $row->datum_pocetka_provere_pk ? Carbon::parse($row->datum_pocetka_provere_pk)->format('d.m.Y') : '',
            $row->datum_pk_izvestaja ? Carbon::parse($row->datum_pk_izvestaja)->format('d.m.Y') : '',
            $row->datum_predaje_dokumentacije ? Carbon::parse($row->datum_predaje_dokumentacije)->format('d.m.Y') : '',
            $row->datum_pocetka_sprovodjenja_intervjua ? Carbon::parse($row->datum_pocetka_sprovodjenja_intervjua)->format('d.m.Y') : '',
            $row->datum_dostavljanja_liste_rukovodiocu_organa ? Carbon::parse($row->datum_dostavljanja_liste_rukovodiocu_organa)->format('d.m.Y') : '',
            $row->datum_donosenja_resenja_o_izabranom_kandidatu ? Carbon::parse($row->datum_donosenja_resenja_o_izabranom_kandidatu)->format('d.m.Y') : '',
            $row->datum_stupanja_na_rad ? Carbon::parse($row->datum_stupanja_na_rad)->format('d.m.Y') : '',
            ...
            $row->broj_usvojenih_zalbi_na_resenje_o_prijemu_u_radni_odnos,
            ...
            $row->datum_formiranja_liste_kandidata ? Carbon::parse($row->datum_formiranja_liste_kandidata)->format('d.m.Y') : '',
